...
function AvailabilityEditor({ initial }: { initial: AvailabilityDto }) {
  ...
  const weeklyMinutes = enabled.reduce(
    (total, s) => total + (s.endMinute - s.startMinute),
    0
  );
  ...
  const handleSave = async () => {
    ...
    try {
      await save.mutateAsync({
        status,
        slots: slots.map(
          ({ dayOfWeek, startMinute, endMinute, enabled: on }) => ({
            dayOfWeek,
            startMinute,
            endMinute,
            enabled: on,
  ...
  return (
    <Stack spacing={3}>
                          ...
                          >
                            <Stack spacing={1.5}>
                              ...
                              >
                                ...
                                />
                                <StatusChip
                                  status={slot.enabled ? "Actif" : "Inactif"}
                                />
                              </Stack>
                            </Stack>
                          </Paper>
      ...
      <InfoPanel title="Conseil">
        Vos créneaux servent à répartir les demandes urgentes : un cardiologue
        en statut « Disponible » et dans une plage active est notifié en
        priorité.
      </InfoPanel>
    </Stack>
  );
