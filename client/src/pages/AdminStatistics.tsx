        ...
        >
          {statistics.data && (
            <StatisticsView
              ...
              palette={palette}
              recentActivity={(audit.data ?? []).slice(0, RECENT_ACTIVITY_LIMIT)}
            />
          )}
        </QueryBoundary>
      </Stack>
...
interface StatisticsViewProps {
  data: Statistics;
  palette: string[];
  recentActivity: { id: string; timestamp: string; user: string; action: string; resource: string }[];
}

function StatisticsView({ data, palette, recentActivity }: StatisticsViewProps) {
  ...
  const stats: { label: string; value: string | number; color: StatColor }[] = [
    ...
    { label: 'Taux de validation', value: formatRate(totals.validationRate), color: 'success' },
    {
      label: 'Temps moyen de validation',
      value: formatDuration(totals.averageReviewDurationMs),
      color: 'info',
    },
    { label: 'Utilisateurs actifs', value: totals.activeUsers, color: 'secondary' },
  ];
  ...
  return (
    <Stack spacing={3}>
          ...
          <SectionCard title="Tendance mensuelle">
            {monthlyDataset.length === 0 ? (
              ...
              <LineChart
                ...
                series={[
                  { dataKey: 'submitted', label: 'Demandes', color: palette[0] },
                  { dataKey: 'validated', label: 'Validées', color: palette[1] },
                ]}
        ...
        <Grid size={{ xs: 12, lg: 4 }}>
          <SectionCard title="Répartition par statut">
            {statusData.length === 0 ? (
              ...
              <PieChart series={[{ data: statusData, innerRadius: 40 }]} height={300} />
            )}
          </SectionCard>
        </Grid>
      </Grid>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, lg: 4 }}>
          <SectionCard title="Répartition par rôle">
            <PieChart series={[{ data: roleData, innerRadius: 40 }]} height={300} />
          </SectionCard>
        </Grid>
      </Grid>
      ...
      <Grid container spacing={3}>
            ...
            {recentActivity.length === 0 ? (
              ...
              <List disablePadding>
                {recentActivity.map((activity, idx) => (
                  <ListItem key={activity.id} divider={idx < recentActivity.length - 1}>
        ...
        <Grid size={{ xs: 12, lg: 6 }}>
          <SectionCard title="Cardiologues les plus actifs" disableContentPadding>
            {topCardiologists.length === 0 ? (
              ...
              <List disablePadding>
                {topCardiologists.slice(0, TOP_CARDIOLOGISTS_LIMIT).map((doc, idx) => (
                  <ListItem
                    key={doc.cardiologistId}
                    divider={idx < Math.min(topCardiologists.length, TOP_CARDIOLOGISTS_LIMIT) - 1}
                  >
                    <ListItemText
                      primary={doc.name}
                    ...
                    />
                    <StatusChip status={formatRate(doc.validationRate)} severity="success" />
                  </ListItem>
                ))}
              </List>
            )}
