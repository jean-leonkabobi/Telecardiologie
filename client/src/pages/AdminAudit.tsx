          ...
          <Stack spacing={3}>
            ...
            {integrity.data && (
              ...
              >
                {integrity.data.intact ? (
                  <>
                    Les {integrity.data.checked} entrées scellées forment une chaîne continue
                    {integrity.data.fromSequence !== null &&
                      ` (rangs ${integrity.data.fromSequence} à ${integrity.data.toSequence})`}
                    . La base refuse toute modification et toute suppression ; ce contrôle
                    détecte en plus une intervention qui aurait désactivé cette protection.
                    {integrity.data.unsealed > 0 &&
                      ` ${integrity.data.unsealed} entrées antérieures au scellement ne sont pas couvertes.`}
                  </>
                ) : (
                  <>
                    La chaîne est rompue à partir du rang {integrity.data.firstBrokenSequence}. Une
                    entrée a été modifiée ou supprimée hors de l'application. À signaler au
