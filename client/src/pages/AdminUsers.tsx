...
export default function AdminUsers() {
  ...
  const stats = {
    ...
    professionals: users.filter(u => u.role === "healthcare_professional")
      .length,
  };
  ...
  const handleDelete = async (user: ManagedUser): Promise<void> => {
    ...
    try {
      ...
      notify.success(
        "Compte supprimé",
        `${user.name} n'a plus accès à la plateforme.`
      );
    } catch (error) {
      notify.error(
        ...
        <DialogContent>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <Typography variant="body2" sx={{ color: "text.secondary" }}>
              Le compte est créé sans mot de passe : l'utilisateur reçoit un
              lien d'activation par email.
            </Typography>
            <TextField
              label="Prénom"
              required
...
function EditUserDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      notify.success(
        "Compte modifié",
        `Les informations de ${user.name} sont à jour.`
      );
      onClose();
    } catch (error) {
      notify.error(
